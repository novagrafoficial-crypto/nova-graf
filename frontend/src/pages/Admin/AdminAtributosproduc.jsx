  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        maxWidth: 360, width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.15)',
      }}>
        <p style={{ margin: '0 0 20px', fontSize: 15, color: '#1e293b' }}>{msg}</p>
...
// ─── estilos ─────────────────────────────────────────────────────────────────
const btn = (v = 'primary') => {
  const base = { border: 'none', borderRadius: 8, padding: '7px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' };
  if (v === 'primary') return { ...base, background: '#1e293b', color: '#fff' };
  if (v === 'ghost')   return { ...base, background: '#f1f5f9', color: '#475569' };
  if (v === 'danger')  return { ...base, background: '#dc2626', color: '#fff' };
  if (v === 'icon')    return { ...base, padding: '4px 9px', fontSize: 12, background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0' };
};
const input = { width: '100%', boxSizing: 'border-box', border: '1px solid #e2e8f0', borderRadius: 8, padding: '7px 11px', fontSize: 13, color: '#1e293b', outline: 'none', background: '#fff' };
const card  = { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '24px 28px', marginBottom: 24 };
const th    = { textAlign: 'left', padding: '7px 10px', color: '#94a3b8', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #f1f5f9' };
const td    = { padding: '9px 10px', color: '#334155', borderBottom: '1px solid #f8fafc', verticalAlign: 'middle' };
...
function TablaSimple({ titulo, endpoint }) {
  ...
  return (
    <>
      ...
      <div style={card}>
        <h2 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: '#1e293b' }}>{titulo}</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filas.length === 0 && (
              <tr><td colSpan={3} style={{ ...td, textAlign: 'center', color: '#94a3b8' }}>Sin registros</td></tr>
            )}
            {filas.map(fila => (
              <tr key={fila.id} style={{ background: editId === fila.id ? '#f8faff' : 'transparent' }}>
                <td style={{ ...td, color: '#94a3b8' }}>{fila.id}</td>
...
function TablaTiposAtributo() {
  ...
  return (
    <>
      ...
      <div style={card}>
        <h2 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: '#1e293b' }}>Tipos de atributo</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filas.length === 0 && (
              <tr><td colSpan={4} style={{ ...td, textAlign: 'center', color: '#94a3b8' }}>Sin registros</td></tr>
            )}
            {filas.map(fila => (
              <tr key={fila.id} style={{ background: editId === fila.id ? '#f8faff' : 'transparent' }}>
                <td style={{ ...td, color: '#94a3b8' }}>{fila.id}</td>
...
function TablaValoresAtributo() {
  ...
  return (
    <>
      ...
      <div style={card}>
        <h2 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: '#1e293b' }}>Valores de atributo</h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          <span style={{ fontSize: 13, color: '#94a3b8' }}>Filtrar por tipo:</span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filasFiltradas.length === 0 && (
              <tr><td colSpan={5} style={{ ...td, textAlign: 'center', color: '#94a3b8' }}>Sin registros</td></tr>
            )}
