...
const C = {
  teal2: "#35BA99", teal1: "#1A6163", gris: "#DBDBDB", rojo: "#FF0000",
  blanco: "#FFFFFF", fondo: "#F4F7F7", fondoCard: "#FFFFFF", texto: "#111111",
  textoSub: "#4A5568", textoMuted: "#9AA5B4", bordeLinea: "#E8ECEF",
};
...
export default function AdminProductos() {
  ...
  const confirmarEliminar = async () => {
    ...
    finally { setModalOpen(false); setProductoAEliminar(null); }
  };
  ...
  return (
    <div style={S.page}>
      <ModalConfirm
        ...
        onCancel={() => { setModalOpen(false); setProductoAEliminar(null); }}
        confirmText="Eliminar"
        cancelText="Cancelar"
        type="danger"
      />
      {vista === "lista" && <ListaProductos productos={productos} loading={loading} error={error} onCrear={() => setVista("crear")} onDetalle={verDetalle} onEditar={abrirEdicion} onEliminar={handleEliminarClick} />}
      {vista === "crear" && <WizardCrear catalogos={catalogos} categorias={categorias} subcategorias={subcategorias} marcas={marcas} onGuardado={() => { cargar(); setVista("lista"); }} onCancelar={() => setVista("lista")} />}
      {vista === "editar" && productoActivo && <EditarProducto producto={productoActivo} categorias={categorias} subcategorias={subcategorias} marcas={marcas} catalogos={catalogos} onGuardado={() => { cargar(); setVista("lista"); }} onCancelar={() => setVista("lista")} />}
      {vista === "detalle" && productoActivo && <DetalleProducto producto={productoActivo} catalogos={catalogos} onVolver={() => { setVista("lista"); setProductoActivo(null); }} />}
    </div>
  );
}
                    ...
                    <tr key={p.id} style={{ background: i % 2 === 0 ? C.blanco : "#FAFCFC" }}>
                      ...
                      <td style={{ ...S.td, fontWeight: 700 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                          {p.imagen_url ? <img src={p.imagen_url} alt="" style={{ width: "36px", height: "36px", objectFit: "cover", borderRadius: "8px" }} /> : <div style={{ width: "36px", height: "36px", background: "#E6F4F4", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px" }}>📷</div>}
                          {p.nombre}
                        </div>
                      </td>
                      ...
                      <td style={{ ...S.td, textAlign: "center" }}><span style={{ background: "#E6F4F4", color: C.teal1, borderRadius: "20px", padding: "3px 12px", fontWeight: 700, fontSize: "0.8rem" }}>{p.num_variantes}</span></td>
...
function EditarProducto({ producto, categorias, subcategorias, marcas, catalogos, onGuardado, onCancelar }) {
  const [form, setForm] = useState({
    ...
    precio_base: producto.precio_base || "", categoria_id: producto.categoria_id || "",
    subcategoria_id: producto.subcategoria_id || "", marca_id: producto.marca_id || "",
  ...
  const guardar = async () => {
    ...
    try {
      const r = await fetch(`${API}/${producto.id}`, {
        ...
        body: JSON.stringify({ ...form, precio_base: parseFloat(form.precio_base), categoria_id: parseInt(form.categoria_id), subcategoria_id: form.subcategoria_id ? parseInt(form.subcategoria_id) : null, marca_id: form.marca_id ? parseInt(form.marca_id) : null, material_id: form.material_id ? parseInt(form.material_id) : null }),
      });
      const d = await r.json();
  ...
  return (
    <>
      <div style={S.header}>
        <div><h1 style={S.title}>Editar producto</h1><p style={{ margin: "2px 0 0", color: C.textoMuted, fontSize: "0.85rem" }}>#{producto.id} · {producto.nombre}</p></div>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginBottom: "1rem" }}><label style={S.label}>Nombre *</label><input style={S.input} value={form.nombre} onChange={e => handle("nombre", e.target.value)} /></div>
        <div style={{ marginBottom: "1rem" }}><label style={S.label}>Descripción</label><textarea style={{ ...S.input, resize: "vertical", minHeight: "80px" }} value={form.descripcion} onChange={e => handle("descripcion", e.target.value)} /></div>
        <div style={{ ...S.grid3, marginBottom: "1rem" }}>
          <div><label style={S.label}>Precio base * ($)</label><input style={S.input} type="number" min="0" step="0.01" value={form.precio_base} onChange={e => handle("precio_base", e.target.value)} /></div>
          <div><label style={S.label}>Categoría *</label><select style={S.select} value={form.categoria_id} onChange={e => { handle("categoria_id", e.target.value); handle("subcategoria_id", ""); }}><option value="">Seleccionar...</option>{categorias.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
          <div><label style={S.label}>Subcategoría</label><select style={S.select} value={form.subcategoria_id} onChange={e => handle("subcategoria_id", e.target.value)} disabled={!form.categoria_id}><option value="">Seleccionar...</option>{subsPorCategoria.map(s => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select></div>
        </div>
        <div style={{ ...S.grid2, marginBottom: "1.5rem" }}>
          <div><label style={S.label}>Marca</label><select style={S.select} value={form.marca_id} onChange={e => handle("marca_id", e.target.value)}><option value="">Sin marca</option>{marcas.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
          <div><label style={S.label}>Material</label><select style={S.select} value={form.material_id} onChange={e => handle("material_id", e.target.value)}><option value="">Sin material</option>{catalogos.materiales.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
        </div>
        <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end" }}>
          <button style={S.btnSecondary} onClick={onCancelar}>Cancelar</button>
          <button style={{ ...S.btnPrimary, opacity: guardando ? 0.6 : 1 }} onClick={guardar} disabled={guardando}>{guardando ? "Guardando..." : "✓ Guardar cambios"}</button>
        </div>
      </div>
    </>
  );
...
function WizardCrear({ catalogos, categorias, subcategorias, marcas, onGuardado, onCancelar }) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      setProgresoSubida("");
      onGuardado();
    } catch (err) { setError(err.message); setProgresoSubida(""); }
    setGuardando(false);
  };
  ...
  return (
    <>
      <div style={S.header}>
        <div><h1 style={S.title}>Nuevo Producto</h1><p style={{ margin: "2px 0 0", color: C.textoMuted, fontSize: "0.85rem" }}>Asistente de creación</p></div>
        <button style={S.btnSecondary} onClick={onCancelar}>← Volver</button>
      </div>
      <div style={S.steps}>
      ...
      {paso === 1 && (
        <div style={S.card}>
          ...
          <div style={{ marginBottom: "1rem" }}><label style={S.label}>Nombre *</label><input style={S.input} placeholder="Ej: Playera personalizada" value={base.nombre} onChange={e => handleBase("nombre", e.target.value)} /></div>
          <div style={{ marginBottom: "1rem" }}><label style={S.label}>Descripción</label><textarea style={{ ...S.input, resize: "vertical", minHeight: "80px" }} placeholder="Descripción..." value={base.descripcion} onChange={e => handleBase("descripcion", e.target.value)} /></div>
          <div style={{ ...S.grid3, marginBottom: "1rem" }}>
            <div><label style={S.label}>Precio base * ($)</label><input style={S.input} type="number" min="0" step="0.01" placeholder="0.00" value={base.precio_base} onChange={e => handleBase("precio_base", e.target.value)} /></div>
            <div><label style={S.label}>Categoría *</label><select style={S.select} value={base.categoria_id} onChange={e => { handleBase("categoria_id", e.target.value); handleBase("subcategoria_id", ""); }}><option value="">Seleccionar...</option>{categorias.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
            <div><label style={S.label}>Subcategoría</label><select style={S.select} value={base.subcategoria_id} onChange={e => handleBase("subcategoria_id", e.target.value)} disabled={!base.categoria_id}><option value="">Seleccionar...</option>{subsPorCategoria.map(s => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select></div>
          </div>
          <div style={{ ...S.grid2, marginBottom: "1.5rem" }}>
            <div><label style={S.label}>Marca</label><select style={S.select} value={base.marca_id} onChange={e => handleBase("marca_id", e.target.value)}><option value="">Sin marca</option>{marcas.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
            <div><label style={S.label}>Material</label><select style={S.select} value={base.material_id} onChange={e => handleBase("material_id", e.target.value)}><option value="">Sin material</option>{catalogos.materiales.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end" }}><button style={S.btnPrimary} onClick={siguientePaso}>Siguiente →</button></div>
        </div>
      )}
      ...
      {paso === 2 && (
        <div style={S.card}>
          ...
          {tiposSeleccionados.length === 0 && <div style={{ background: "#FFF8E6", border: "1px solid #F5D87A", borderRadius: "10px", padding: "12px 16px", color: "#8A6000", fontSize: "0.88rem", marginBottom: "1.5rem" }}>⚠️ Sin atributos seleccionados. Solo podrás elegir colores.</div>}
          <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end" }}>
      ...
      {paso === 3 && (
        <div>
          ...
          <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
            <button style={{ ...S.btnPrimary, padding: "14px 36px", fontSize: "1rem", background: C.teal2 }} onClick={generarCombinaciones}>⚡ Generar combinaciones automáticamente</button>
            <p style={{ color: C.textoMuted, fontSize: "0.82rem", marginTop: "8px" }}>Se generarán <strong style={{ color: C.teal1 }}>{totalCombinaciones}</strong> variante(s)</p>
          </div>
          ...
          <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "1rem" }}>
            ...
            <button style={{ ...S.btnPrimary, opacity: guardando ? 0.6 : 1 }} onClick={guardar} disabled={guardando}>{guardando ? "Guardando..." : "✓ Guardar producto"}</button>
          </div>
        </div>
      )}
    </>
...
function DetalleProducto({ producto: productoInicial, catalogos, onVolver }) {
  ...
  const actualizarStock = async (varianteId, cantidad) => {
    ...
    if (r.ok) { setStockEdit({}); await recargar(); mostrarExito("Stock actualizado"); }
    else { mostrarError("Error al actualizar stock"); }
  };
  ...
  return (
    <>
      <div style={S.header}>
        <div><h1 style={S.title}>{producto.nombre}</h1><p style={{ margin: "2px 0 0", color: C.textoMuted, fontSize: "0.85rem" }}>Detalle · #{producto.id}</p></div>
                    ...
                    return (
                      <tr key={v.id} style={{ background: i % 2 === 0 ? C.blanco : "#FAFCFC" }}>
                        ...
                        <td style={{ ...S.td, fontFamily: "monospace", fontSize: "0.82rem" }}>
                          {editando ? <input style={{ ...S.input, padding: "5px 8px", width: "140px", fontFamily: "monospace" }} value={varEditForm.sku} onChange={e => setVarEditForm(f => ({ ...f, sku: e.target.value.toUpperCase() }))} /> : v.sku}
                        </td>
                        <td style={S.td}>
                          {editando
                            ? <select style={{ ...S.select, padding: "5px 8px", width: "130px" }} value={varEditForm.color_id} onChange={e => setVarEditForm(f => ({ ...f, color_id: e.target.value }))}><option value="">Sin color</option>{catalogos.colores.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
                            : v.color_nombre || "—"}
                        </td>
                        <td style={S.td}>
                          {v.atributos?.map(a => (
                        ...
                        <td style={{ ...S.td, fontWeight: 700, color: C.teal1 }}>
                          {editando
                            ? <div><input type="number" step="0.01" style={{ ...S.input, padding: "5px 8px", width: "90px" }} value={varEditForm.precio_adicional} onChange={e => setVarEditForm(f => ({ ...f, precio_adicional: e.target.value }))} /><div style={{ fontSize: "0.72rem", color: C.textoMuted, marginTop: "2px" }}>Total: ${(parseFloat(producto.precio_base) + parseFloat(varEditForm.precio_adicional || 0)).toFixed(2)}</div></div>
