...
const Checkout = () => {
  ...
  const handleContinuarEntrega = () => {
    ...
    if (formaEntrega === 'domicilio' && !direccion.trim()) {
      alert('Ingresa tu dirección completa');
      return;
    }
    setPaso('pago');
  };

  // Simular compra y guardar pedido en localStorage
  const handleSimularCompra = (e) => {
    ...
    setTimeout(() => {
      alert('✅ ¡Compra simulada con éxito! Revisa "Mis compras".');
      refreshCart(); // actualiza el badge del carrito
        ...
        <div className="checkout-pago-container">
          ...
          <form onSubmit={handleSimularCompra} className="pago-form">
            ...
            <button type="submit" disabled={procesando} className="btn-pagar">
              {procesando ? 'Procesando...' : 'Confirmar compra (simulación)'}
            </button>
