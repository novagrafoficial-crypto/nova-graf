  ...
  const aprobarPropuesta = async (solicitudId, propuestaId, costoDiseno, varianteId, precioBase) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/client/solicitudes-diseno/${solicitudId}/aprobar`,
        {
          method: 'POST',
          ...
          return (
            <div key={sol.id} className="sd-card">
              ...
              {sol.variante && (
                <div className="sd-producto">
                  ...
                  <div className="sd-producto__info">
                    ...
                    {sol.variante.color && (
                      <span className="sd-producto__detalle">
                        <span className="sd-label">Color:</span> {sol.variante.color}
                      </span>
                    )}
                    <span className="sd-producto__detalle">
                      <span className="sd-label">Precio base:</span>{' '}
                      ${parseFloat(sol.variante.precio_base).toFixed(2)}
                    </span>
                    {sol.costo_diseno && (
                      <span className="sd-producto__detalle sd-costo-diseno">
                        <span className="sd-label">Costo diseño:</span>{' '}
                        ${parseFloat(sol.costo_diseno).toFixed(2)}
                      </span>
                    )}
                    {sol.costo_diseno && sol.variante.precio_base && (
                      <span className="sd-producto__detalle sd-total">
                        <span className="sd-label">Total estimado:</span>{' '}
                        ${(parseFloat(sol.variante.precio_base) + parseFloat(sol.costo_diseno)).toFixed(2)}
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="sd-seccion">
                <p className="sd-label">Tu descripción</p>
